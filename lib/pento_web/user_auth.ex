defmodule PentoWeb.UserAuth do
  ...
  @spec log_in_user(Plug.Conn.t(), any(), any()) :: Plug.Conn.t()
  @doc """
  Logs the user in.
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      PentoWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule PentoWeb.PageLive do
        ...
        on_mount {PentoWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{PentoWeb.UserAuth, :require_authenticated}] do
